...
myDiffExpDDS <- function(dds,colData1,colData2,formulaEdgR,formulaDE2,ref,name,org,anno,db) {
  library("edgeR")
  library("DESeq2")
  #library("stringr")
  library("gplots")
  ...
  pca12 <- ggplot(data = loadings,
                  aes(x=PC1,
                      y=PC2,
                      color=as.factor(colData(dds)[[colData1]]),
                      shape=as.factor(colData(dds)[[colData2]])))
  print(pca12 +
          geom_point(size=17) +
          scale_shape_manual(values=seq(21, 21+length(levels(as.factor(colData(dds)[[colData2]])))-1, 1)) +
          theme_bw() +
          ...
          geom_text(aes(label=row.names(colData(dds)))) +
  ...
  if (!is.null(formulaEdgR)) {
    ...
    y <- DGEList(counts=counts(dds,normalized=F),group=colData(dds)[,colData1])
    y$samples$group <- relevel(y$samples$group, ref=ref)
    #y <- calcNormFactors(y)
    y <- calcNormFactors(y,method="TMM")
    
    #output normalized count table
    edegeRExp <- cpm(y)
    ...
    qlf <- glmQLFTest(fit)#,coef=1)#coef=1#, lfc=1)
    topTags(qlf)
    ...
    if(stringr::str_starts(row.names(as.data.frame(res))[1],"NM") == TRUE){
    ...
    sapply(db,webG_X <- function(x) {
      webG_2(counts(dds,normalized=F),resSig,org,x,name,gsea.df)})
  } else {
    print("#########################################################")
    ...
    res <- results(dds,alpha=0.05, lfcThreshold=log2(2),name="Group_FT_vs_FA")
    ...
    if(dim(resSigUp)[1] > 10 |  dim(resSigDown)[1] > 10){
      sapply(db,webG_X <- function(x) {
        webG_2(counts(dds,normalized=F),resSig,org,x,name,gsea.df)})
    } else {print("Not enough significant results to run GO term analysis.")}
  }
  graphics.off()
  return(results)
}
