...
webG <- function(counts,resSig,org,db,name,gsea.df=NULL){
  ...
  if(!is.null(resSig$logFC)){
    ...
    if (!is.null(dim(Up.GSEA)[1])) {
      ...
      Up.GSEA.Sig <- Up.GSEA.Sig[order(Up.GSEA.Sig$NES, decreasing= F),]
      Up.GSEA.Sig$labs <- factor(Up.GSEA.Sig$labs, levels = Up.GSEA.Sig$labs[order(Up.GSEA.Sig$NES, decreasing= F)])
      p.Up.GSEA <- ggplot(data=na.omit(Up.GSEA.Sig[1:50,]), aes(x=labs, y=NES, fill=NES)) +
        geom_bar(stat="identity",col="black") +
        scale_fill_gradient(low="lightskyblue", high="tomato") +
        geom_text(aes(label=signif(FDR, digits = 2)), hjust=1.1, color="white", size=3)+
    ...
    Up.GSEA <- tryCatch(WebGestaltR(enrichMethod="GSEA",
                                    ...
                                    interestGeneType="refseq_mrna",
                                    projectName=sprintf("%s.%s.GSEA",name,db),
                                    interestGene=gsea.df, nThreads=10 ), error = function(err) {
                                      # error handler picks up where error was generated
                                      print(paste("MY_ERROR:  ",err))
                                      Up.GSEA=NULL
                                      return(Up.GSEA)
    ...
    if (!is.null(dim(Up.GSEA)[1])) {
      ...
      ggsave(filename = sprintf("%s_%s_GSEA_DESeq.pdf",name,db), plot = p.Up.GSEA, width = 11, height = 11)
    }
    ...
    if(dim(resSig[resSig$avg_logFC > 0,])[1] > 10){
      ...
      Up.ORA <- WebGestaltR(enrichMethod="ORA",
                            ...
                            projectName=sprintf("%s.%s.Up.ORA",name,db),
                            referenceGene=row.names(gsea.df),
                            interestGene=row.names(resSig[resSig$avg_logFC > 0,]))
      if (!is.null(dim(Up.ORA)[1])) {
    ...
    if(dim(gsea.df)[1] > 10){
      ...
      Up.GSEA <- tryCatch(WebGestaltR(enrichMethod="GSEA",
                                      ...
                                      interestGeneType="genesymbol",
                                      projectName=sprintf("%s.%s.GSEA",name,db),
                                      interestGene=ddd ), error = function(err) {
                                        # error handler picks up where error was generated
                                        print(paste("MY_ERROR:  ",err))
                                        Up.GSEA=NULL
                                        return(Up.GSEA)
