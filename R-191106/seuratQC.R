...
seuratQC <- function(ids, nam, pmito = 0.1, nf_max = 6000, nf_min = 200){
  library(Seurat)
  library(ggplot2)
  ...
  print("...subsetting...")
  nf_minx <- substitute(nf_min)
  nf_maxx <- substitute(nf_max)
  pmitox <- substitute(pmito)
  seurat.obj <- subset(seurat.obj, nFeature_RNA > eval(quote(nf_minx),parent.frame()) & nFeature_RNA < eval(quote(nf_maxx),parent.frame()) & percent.mito < eval(quote(pmitox),parent.frame()))
